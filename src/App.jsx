...
export default function App() {
  ...
  useEffect(() => {
    if (isTrialActive && workspace?.id) {
      const key = `trial_tracked_${workspace.id}`;
      if (!sessionStorage.getItem(key)) {
        trackTrialStarted();
        sessionStorage.setItem(key, "1");
      }
    }
  }, [isTrialActive, workspace?.id]);
  ...
  const handleNewProject     = async () => { if (!canCreateProyecto()) { openPaywall("maxProyectos"); return; } const id = await newProyecto(); if (id) { dispatch({ type:"NEW_PROJECT", payload:id }); setTab(2); trackProyectoCreado(id); } };
  ...
  // apre automaticamente WhatsApp precompilato dopo un breve delay (UX: stato prima, WA dopo)
  const handleSetEstado = useCallback((nuovoEstado) => {
    setEstado(nuovoEstado);
    if (!WA_ESTADOS.has(nuovoEstado)) return;
    const link = waLink(
      proy.info.telefono,
      nuovoEstado,
      {
        cliente:     proy.info.cliente,
        descripcion: proy.info.descripcion,
        total:       totals.totalIva || totals.total,
        empresa:     workspace?.name,
      }
    );
    if (link) {
      // Piccolo delay così il re-render dello stato è già visibile prima di aprire WA
  ...
  const handleInviaFirma = async () => {
    ...
    if (result) { copiaLink(result.url); trackFirmaSolicitada(); }
  };
    ...
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#f0f4f8",minHeight:"100vh",
      ...
      {paywallFeature && (
        <PaywallModal
          ...
          onGoToPiani={() => { closePaywall(); setTab(15); trackUpgradeClick("paywall"); }}
        />
      )}
      ...
      {isMobile && proy.info.cliente && (
        <div className="no-print" style={{ background:"white",padding:"8px 14px",display:"flex",gap:8,alignItems:"center",borderBottom:"1px solid #e2e8f0" }}>
          ...
          <button onClick={() => {
            ...
            handleSetEstado(ESTADOS[(idx+1) % ESTADOS.length]);
